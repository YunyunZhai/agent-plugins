...
### A/B 重叠率（`compare_models.py` 脚本 + `data/compare_report*.json`）
...
- 最终产出：43,2586 向量（`embed_full.log` 显示 69,590 嵌入 / 累计 1,246,139 tokens / 总向量 432,586）。

证据：`skills/gh-search/scripts/embed_full.log`、`architecture.md`。
...
- 深窗口 kNN k=4000（vec0 全库暴力扫描零额外成本），保证头部项目先进候选池。
...
证据：`skills/data/semantic_search.log`（kNN 4000 → merge 7639，new_from_readme=3639）。
...
证据：`skills/data/search_repos.log`。
...
证据：`skills/data/rerank_results.log`（4 行，跳过 rerank）、`service/billing.py`。
...
| 日志 | `data/*.log`、`skills/data/*.log`、`skills/gh-search/scripts/*.log` | 抓取/嵌入/查询/限速记录 |
