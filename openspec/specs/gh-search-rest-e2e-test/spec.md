# gh-search-rest-e2e-test Specification

## Purpose

为 gh-search REST 服务提供可复用的端到端测试契约，覆盖 `/health`、`/search`（keyword/semantic/hybrid 与 enrich/readme/rerank 参数）、`/billing/summary` 与非法参数校验，并约束测试运行说明与环境变量配置文档。
