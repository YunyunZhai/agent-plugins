...
设计要点:
    - 数据库文件默认放插件目录 `gh-search/data/gh_search_index.db`
...
# 默认数据库路径：插件 data 目录（gh-search/data/）
# 脚本位于 gh-search/scripts/_common/ → 上 3 层到 gh-search/
_DEFAULT_DB = (
    Path(__file__).resolve().parent.parent.parent / "data" / "gh_search_index_v3.db"
)
