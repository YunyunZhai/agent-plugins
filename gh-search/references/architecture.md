...
| `_common/logsetup.py` | 共享轮转日志（5MB×3）到 `data/<logger>.log` |

---

## 五、数据库 schema

默认库：`gh-search/data/gh_search_index_v3.db`（`GH_SEARCH_DB` 可覆盖）。
...
```bash
# 语义查询（生产姿势）
GH_SEARCH_BACKEND=local GH_SEARCH_DB=gh-search/data/gh_search_index_v3.db \
