...
```
【建库（一次性/每周增量）】
GitHub GraphQL ──data/fetch_repos.py──▶ sqlite repos 表
...
README 双通道（2026-08-25 启用）：data/fetch_readmes.py 抓取清洗（stars≥2000，
共 30,378 条）→ 同一 Kaggle T4 管线嵌入 → sqlite-vec repo_readme_vectors

【查询（search/semantic_search.py --backend local）】
...
| 文件 | 角色 |
|------|------|
| `scripts/pipeline/build_index.py` | 批量嵌入入口。`--backend local/ark/pinecone`，`--shard i:n` 多进程分片 |
| `scripts/pipeline/import_gpu_vectors.py` | Kaggle npz 回导 v3 库（校验维度/归一化/id 完整性，幂等 DELETE+INSERT） |
| `scripts/search/semantic_search.py` | 查询入口。`--backend local` + `--star-weight 0.03`(默认) + `--dual-query`/`--pure-semantic` 开关 |
| `scripts/data/fetch_repos.py` | 元数据抓取 + `--sync-stars` 星数快照同步（REST 自适应区间，30 req/min 限速内） |
| `scripts/_common/sqlite_store.py` | schema 与 vec0 封装。EMBED_DIM 由 `GH_SEARCH_EMBED_DIM` 控制（v3 用默认 1024） |
| `scripts/_common/ark_client.py` | 方舟 chat/embeddings 客户端（备用后端；强制 IPv4 绕 fake-ip 故障） |
...
python3 scripts/data/fetch_repos.py --sync-stars --db <v3库>
